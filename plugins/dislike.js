...
if (new Date - global.DATABASE._data.users[m.sender].lastme > 40400) {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `a qien quieres dar dislike`, m)
...
handler.help = ['dislike  *tag*']
handler.tags = ['main']
handler.command = /^(dislike|nomg)$/i
handler.admin = false
handler.group = true
