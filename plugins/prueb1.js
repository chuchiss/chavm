...
klik wa.me/${conn.user.jid.split`@`[0]}`, m.chat, {
      ...
      if (!room.pilih) return m.reply(`Silahkan pilih`)
      if (!room.pilih2) return return m.reply(`Silahkan pilih`)
     room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) this.reply(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`)
         ...
         global.DATABASE.data.users[win == room.p ? room.p : room.p2].exp += room.poin
          global.DATABASE.data.users[win == room.p ? room.p2 : room.p].exp -= room.poin_lose
        }
        delete this.suit[room.id]
        return !0
      }, room.timeout)
