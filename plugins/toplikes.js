let handler = async (m, { conn, args }) => {
  ...
  let sortedExp = Object.entries(global.DATABASE.data.users).sort((a, b) => b[1].preg - a[1].preg)
...
${sortedExp.slice(0, len).map(([user, data], i) => (i + 1) + '. @' + user.split`@`[0] + ': *' + data.preg + ' ✔️*').join`\n`}
